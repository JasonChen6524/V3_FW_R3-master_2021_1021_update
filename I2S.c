...
I16 dummy;
    ...
    if (v3sense)  //  current sensing in effect read rx buffers
    {
      if (USART_StatusGet(USART0) & USART_STATUS_RXDATAV)
      {

          if (USART_StatusGet(USART0) & USART_STATUS_RXDATAVRIGHT)
          {
             dummy  = 0;
             if ((USART0->STATUS & USART_STATUS_RXFULL)) dummy = (uint16_t)USART0->RXDOUBLE;
             if (sinosc[0].sample < dummy) sinosc[0].sample = dummy;          

             dummy  = 0;
             if ((USART2->STATUS & USART_STATUS_RXFULL)) dummy = (uint16_t)USART2->RXDOUBLE;
             if (sinosc[2].sample < dummy) sinosc[2].sample = dummy;          
          } else
          {
             dummy  = 0;
             if ((USART0->STATUS & USART_STATUS_RXFULL)) dummy = (uint16_t)USART0->RXDOUBLE;
             if (sinosc[1].sample < dummy) sinosc[1].sample = dummy;

             dummy  = 0;
             if ((USART2->STATUS & USART_STATUS_RXFULL)) dummy = (uint16_t)USART2->RXDOUBLE;
             if (sinosc[3].sample < dummy) sinosc[3].sample = dummy;         
          }
...
void GetSinParams(void)
{
U8 i;

   for(i=0;i<4;i++)
   {
	   sinosc[i].w0 = 2.0 * V3PI *  ((double) sinosc[i].hzfreq/SINSFRQ);
	   sinosc[i].a1 = -2.0 * cos((double) sinosc[i].w0);
	   sinosc[i].y2 = SinGain * sin((double) sinosc[i].w0);  // SinGain is inverted when set
	   sinosc[i].y1 = 0;      
   }
}
